...
type SortField = 'name' | 'age' | 'metAt' | 'gender' | 'height' | 'occupation' | 'meetingPlace';
  ...
  return (
    <div className={styles.row} onClick={() => onClick(contact.id)}>
      <div className={styles.cellName}>
        <Avatar size={28} icon={<UserOutlined />} src={contact.photo} className={styles.avatar} />
      ...
      <div className={styles.cellSmall}>
        {contact.age ? `${contact.ageType === 'approximate' ? '~' : ''}${contact.age}` : '-'}
      </div>
      <div className={styles.cellSmall}>
        {contact.height ? `${contact.heightType === 'approximate' ? '~' : ''}${contact.height}` : '-'}
      </div>
          ...
          <Tooltip title={tagsTooltip} mouseEnterDelay={0.5}>
            <div className={styles.tagsWrapper}>
              ...
              {contact.tags.length > 2 && (
                <span className={styles.moreText}>+{contact.tags.length - 2}</span>
              )}
            </div>
          </Tooltip>
        ) : '-'}
...
const SortIconDesc = () => <CaretDownOutlined className={styles.sortIconActive} />;

export const ContactsTable = memo(({ contacts, filters, onFiltersChange, onRowClick, loading }: ContactsTableProps) => {
  const handleSort = useCallback((field: SortField) => {
    const isSameField = filters.sortBy === field;
    const newOrder = isSameField
      ? filters.sortOrder === 'asc' ? 'desc' : filters.sortOrder === 'desc' ? undefined : 'asc'
      : 'asc';
  ...
  const occupationSortIcon = useMemo(() => getSortIcon('occupation'), [getSortIcon]);
  const meetingPlaceSortIcon = useMemo(() => getSortIcon('meetingPlace'), [getSortIcon]);
  ...
  const handleSortGender = useCallback(() => handleSort('gender'), [handleSort]);
  const handleSortAge = useCallback(() => handleSort('age'), [handleSort]);
  const handleSortHeight = useCallback(() => handleSort('height'), [handleSort]);
  const handleSortOccupation = useCallback(() => handleSort('occupation'), [handleSort]);
  const handleSortMeetingPlace = useCallback(() => handleSort('meetingPlace'), [handleSort]);
  const handleSortMetAt = useCallback(() => handleSort('metAt'), [handleSort]);
  ...
  return (
    <div className={styles.tableContainer}>
      ...
      <div className={styles.headerRow}>
        <div className={`${styles.headerCell} ${styles.sortable}`} onClick={handleSortName}>
          Имя {nameSortIcon}
        </div>
        <div className={`${styles.headerCell} ${styles.sortable}`} onClick={handleSortGender}>
          Пол {genderSortIcon}
        </div>
        <div className={`${styles.headerCell} ${styles.sortable}`} onClick={handleSortAge}>
          Возраст {ageSortIcon}
        </div>
        <div className={`${styles.headerCell} ${styles.sortable}`} onClick={handleSortHeight}>
          Рост {heightSortIcon}
        </div>
        <div className={`${styles.headerCell} ${styles.sortable}`} onClick={handleSortOccupation}>
          Профессия {occupationSortIcon}
        </div>
        <div className={`${styles.headerCell} ${styles.sortable}`} onClick={handleSortMeetingPlace}>
          Место {meetingPlaceSortIcon}
        </div>
        <div className={`${styles.headerCell} ${styles.sortable}`} onClick={handleSortMetAt}>
          Дата {metAtSortIcon}
        </div>
